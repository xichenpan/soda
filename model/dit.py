...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.x_embedder = PatchEmbed(input_size, patch_size, in_channels,
                                     hidden_size)  # timm 3.2 doesn't have bias arg.
...
class DiTLLaMA(DiT):
    def __init__(
        ...
        super().__init__(input_size, patch_size, in_channels, hidden_size, depth, num_heads, mlp_ratio,
...
class BottleneckDiTLLaMA(nn.Module):
    def __init__(
        ...
        self.t_embedder = TimestepEmbedder(hidden_size)
        self.z_embedder = LatentEmbedder(z_channels)
        num_patches = self.x_embedder.num_patches
        # Will use fixed sin-cos embedding:
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, hidden_size), requires_grad=False)
    ...
    def forward(self, x, t, z, drop_mask):
        """
    ...
    def forward_with_cfg(self, x, t, z, drop_mask, cfg_scale):
        """
        Forward pass of DiT, but also batches the unconditional forward pass for classifier-free guidance.
        """
        # https://github.com/openai/glide-text2im/blob/main/notebooks/text2im.ipynb
        half = x[: len(x) // 2]
        combined = torch.cat([half, half], dim=0)
        model_out = self.forward(combined, t, z, drop_mask)
        # For exact reproducibility reasons, we apply classifier-free guidance on only
        # three channels by default. The standard approach to cfg applies it to all channels.
        # This can be done by uncommenting the following line and commenting-out the line following that.
        # eps, rest = model_out[:, :self.in_channels], model_out[:, self.in_channels:]
...
def BottleneckDiTLLaMA_XL_2(**kwargs):
    return BottleneckDiTLLaMA(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
...
DiT_models = {
    ...
    'DiTLLaMA_XL_4': DiTLLaMA_XL_4, 'BottleneckDiTLLaMA_XL_2': BottleneckDiTLLaMA_XL_2
}
